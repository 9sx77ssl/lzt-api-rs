...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token = std::env::var("LZT_API_TOKEN").expect("Set LZT_API_TOKEN");
    ...
    test!(
        "categories_list",
        client.forum().categories_list(Default::default())
    );
    test!(
        "forums_list",
        client.forum().forums_list(Default::default())
    );
    test!(
        "threads_list",
    ...
    test!(
        "users_get",
        client.forum().users_get(json!(1), Default::default())
    );
    test!(
        "conversations_list",
        client.forum().conversations_list(Default::default())
    );
    test!(
        "chatbox_index",
        client.forum().chatbox_index(Default::default())
    );
    test!("tags_list", client.forum().tags_list(Default::default()));
    // navigation_list может вернуть 500 - ретраим
    print!("navigation_list... ");
    match client.forum().navigation_list(Default::default()).await {
        Ok(_) => {
            println!("✓");
            passed += 1;
        }
    ...
    test!("pages_list", client.forum().pages_list(Default::default()));
    test!(
        "assets_css",
        client
            .forum()
            .assets_css(forum::AssetsCssQuery { css: Some(vec![]) })
    );
    ...
    test!(
        "profile_get",
        client.market().profile_get(Default::default())
    );
